...
parser.add_argument("-t",dest="tag",action="store",required=True,help="Tag that will be used before each outfileName")
...
outfile = open(args.tag+".BRH.txt","w")
for code in pairs:
	print >>outfile,code.replace("-","\t")
outfile.close()

outfile = open(args.tag+".unpaired.txt","w")
for code in seqs1:
