...
$blast_two_passes = 2;  # Set to 1 to run 2-pass strategy                     #
...
$blastall = "blastp";  #Add -aN to use N processors
...
$mysql_table = 0; # Print out sql tables for the web server                    #
...
if ($run_blast){
    ...
    system ("$formatdb -dbtype prot -in $fasta_seq_fileA");
    system ("$formatdb -dbtype prot -in $fasta_seq_fileB") if (@ARGV >= 2);
    system ("$formatdb -dbtype prot -in $fasta_seq_fileC") if ($use_outgroup);   
...
sub bootstrap{
    ...
    system "$formatdb -dbtype prot -in $db_file";
    # Use soft masking in 1-pass mode for simplicity.
    system "$blastall -query $query_file -db $db_file -matrix $matrix -outfmt 5 | ./$blastParser 0 -a > $seq_id2.faa";
    ...
    unlink "$seq_id2.fas.psq", "$seq_id2.fas.pin", "$seq_id2.fas.phr";
...
sub do_blast_1pass {
  ...
  system ("$blastall -query $Fld[0] -db $Fld[1] -num_alignments $Fld[3] -matrix $matrix -outfmt 5 | ./$blastParser $score_cutoff > $Fld[4]");
}
...
sub do_blast_2pass {
	...
	system("date");
	open FHR, "$blastall -comp_based_stats 3 -query $Fld[0] -db $Fld[1] -num_alignments $Fld[3] -matrix $matrix -outfmt 5 | ./$blastParser $score_cutoff|";
	...
	foreach $aQuery (keys % theHits) {
		...
		system ("$formatdb -dbtype prot -in $tmpd");
		system ("$blastall -comp_based_stats 0 -query $tmpi -db $tmpd -num_alignments $Fld[3] -matrix $matrix -outfmt 5 | ./$blastParser $score_cutoff >> $Fld[4]");
	}

	unlink "$tmpi", "$tmpd", "$tmpd.phr", "$tmpd.pin", "$tmpd.psq";
}
